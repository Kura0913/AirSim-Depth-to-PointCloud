...
class MysqlDBInitializer:
    ...
    def get_existing_tables(self):
        ...
        return [list(single_row.values())[0] for single_row in records]
    ...
    def create_not_exist_table(self, existing_tables):
        for necessary_table, table_creating_command in necessary_table_to_create.items():
            if necessary_table not in existing_tables:
                self.create_table_with_specified_command(table_creating_command)

    def create_table_with_specified_command(self, command):
        with DBConnection() as connection:
